...
def load_config(config_path='config.json'):
    ...
    default_config = {
        ...
        # 高级风格迁移配置
        'latent_adain_scale': 0.5 # 激活Latent AdaIN
    }
...
class TwoStageImageEnhancer:
    """
    两阶段图像增强器 - 让贴着的细节"隆起来"
    
    阶段1: 图像预处理增强 - 让文字等细节产生高度差
    阶段2: 深度感知优化 - 确保增强效果能被depth模型捕捉
    """
    
    def __init__(self, 
                 text_enhancement_factor=3.0,
                 edge_enhancement_factor=2.5,
                 detail_amplification=1.8,
                 structure_boost=2.0):
        """初始化两阶段增强器"""
        self.text_enhancement_factor = text_enhancement_factor
        self.edge_enhancement_factor = edge_enhancement_factor
        self.detail_amplification = detail_amplification
        self.structure_boost = structure_boost
        
        print(f"🔧 增强器参数: text={text_enhancement_factor}, edge={edge_enhancement_factor}, detail={detail_amplification}, boost={structure_boost}")
        ...
        # 文字检测核
        self.text_kernel_h = np.array([[-1, -1, -1], [2, 2, 2], [-1, -1, -1]], dtype=np.float32)
        self.text_kernel_v = np.array([[-1, 2, -1], [-1, 2, -1], [-1, 2, -1]], dtype=np.float32)
    
    def enhance_geometry_for_depth(self, image):
        """
        有效的结构增强 - 让贴着的细节真正"隆起来"
        
        策略：基于亮度对比度进行有意义的结构增强
        
        参数:
            image: PIL Image 或 numpy array
            
        返回:
            geometry_enhanced_image: 几何增强后的图像（用于depth估计）
        """
        ...
        if img_array.ndim == 3:
            ...
            gray = img_array.astype(np.float32) / 255.0
        
        # 🎯 新策略：基于亮度对比度的有效增强
        
        # 1. 检测亮度对比区域 - 这些通常是文字、线条等结构
        # 使用简单的局部对比度计算
        h, w = gray.shape
        ...
        for i in range(half_k, h - half_k):
            for j in range(half_k, w - half_k):
                # 获取邻域
                neighborhood = gray[i-half_k:i+half_k+1, j-half_k:j+half_k+1]
                center_value = gray[i, j]
                
                # 计算对比度：中心像素与邻域平均值的差异
                neighborhood_mean = np.mean(neighborhood)
                contrast = abs(center_value - neighborhood_mean)
                contrast_map[i, j] = contrast
        
        # 2. 基于对比度强度进行分级增强
        # 获取增强参数 - 大幅提高增强强度
        enhancement_strength = self.text_enhancement_factor * 0.05  # 提高增强强度
        
        print(f"🔧 实际增强强度: {enhancement_strength}")
        
        # 对比度阈值：只增强有明显对比度的区域
        contrast_threshold = np.percentile(contrast_map, 80)  # 前20%的对比度区域
        
        # 创建增强掩码
        enhancement_mask = (contrast_map > contrast_threshold).astype(np.float32)
        
        # 3. 根据对比度强度计算增强量
        # 归一化对比度
        normalized_contrast = contrast_map / (np.max(contrast_map) + 1e-10)
        
        # 计算增强量：对比度越强，增强越多
        enhancement_amount = enhancement_mask * normalized_contrast * enhancement_strength
        ...
        mean_enhancement = np.mean(enhancement_amount[enhancement_amount > 0])
        enhanced_pixels = np.sum(enhancement_amount > 0)
        total_pixels = enhancement_amount.size
        
        print(f"🔧 几何增强统计:")
        print(f"   最大增强值: {max_enhancement:.4f}")
        print(f"   平均增强值: {mean_enhancement:.4f}")
        print(f"   增强像素数: {enhanced_pixels}/{total_pixels} ({enhanced_pixels/total_pixels*100:.1f}%)")
        
        # 激进的亮度调整策略
        brightness_factor = np.where(gray < 0.5, 
                                   1.0 + enhancement_amount * 3.0,  # 暗区域大幅增亮
                                   1.0 + enhancement_amount * 1.5)  # 亮区域也显著增强
        
        # 5. 应用增强到图像
        if img_array.ndim == 3:
            enhanced_color = img_array.astype(np.float32) / 255.0
            
            # 对每个颜色通道应用增强
            for c in range(3):
                enhanced_color[:, :, c] *= brightness_factor
            
            # 确保值在有效范围内
            enhanced_color = np.clip(enhanced_color, 0, 1)
            enhanced_color = (enhanced_color * 255).astype(np.uint8)
            
            return Image.fromarray(enhanced_color) if isinstance(image, Image.Image) else enhanced_color
        else:
            # 灰度图处理
            enhanced_gray = gray * brightness_factor
            enhanced_gray = np.clip(enhanced_gray, 0, 1)
            enhanced_gray = (enhanced_gray * 255).astype(np.uint8)
            
            return Image.fromarray(enhanced_gray) if isinstance(image, Image.Image) else enhanced_gray
...
def wct_color_transfer(content_img, style_img):

    # 转为 LAB 色彩空间
    content = cv2.cvtColor(np.array(content_img), cv2.COLOR_RGB2LAB).astype(np.float32)
    style = cv2.cvtColor(np.array(style_img), cv2.COLOR_RGB2LAB).astype(np.float32)

    # 转为 shape: [3, H*W]
    c_flat = content.reshape(-1, 3).T
    s_flat = style.reshape(-1, 3).T

    # 减均值
    c_mean = np.mean(c_flat, axis=1, keepdims=True)
    s_mean = np.mean(s_flat, axis=1, keepdims=True)
    c_centered = c_flat - c_mean
    s_centered = s_flat - s_mean

    # 计算协方差
    c_cov = np.cov(c_centered)
    s_cov = np.cov(s_centered)

    # 奇异值分解（白化 + 着色）
    U_c, S_c, _ = np.linalg.svd(c_cov + np.eye(3) * 1e-5)
    U_s, S_s, _ = np.linalg.svd(s_cov + np.eye(3) * 1e-5)

    # 白化
    whiten = U_c @ np.diag(1.0 / np.sqrt(S_c)) @ U_c.T @ c_centered
    # 着色
    colored = U_s @ np.diag(np.sqrt(S_s)) @ U_s.T @ whiten + s_mean

    # 还原图像
    result = colored.T.reshape(content.shape)
    result = np.clip(result, 0, 255).astype(np.uint8)
    result_rgb = cv2.cvtColor(result, cv2.COLOR_LAB2RGB)
    return Image.fromarray(result_rgb)

def partial_desaturate(img: Image.Image, alpha=0.1):
    """
    alpha=0 表示全灰度，alpha=1 表示保留原色
    """
    gray = img.convert('L').convert('RGB')
    img_np = np.array(img).astype(np.float32)
    gray_np = np.array(gray).astype(np.float32)
    blend_np = gray_np * (1 - alpha) + img_np * alpha
...
def main():
    ...
    # 🚀 新增：初始化两阶段图像增强器 - 有效的增强参数
    image_enhancer = TwoStageImageEnhancer(
        text_enhancement_factor=config_dict.get('text_enhancement_factor', 5.0),  # 提高文字增强
        edge_enhancement_factor=config_dict.get('edge_enhancement_factor', 3.0),  # 提高边缘增强
        detail_amplification=config_dict.get('detail_amplification', 2.0),       # 提高细节放大
        structure_boost=config_dict.get('structure_boost', 2.5)                  # 提高结构提升
    )
    
    # 🔄 阶段1：几何增强 - 让贴着的细节产生高度差（无光照效果）

    target_image_original = Image.open(obj_image_path).convert('RGB')
    geometry_enhanced_image = image_enhancer.enhance_geometry_for_depth(target_image_original)
    
    # 保存几何增强后的图像用于调试
    geometry_enhanced_image.save("geometry_enhanced_for_depth.png")
    ...
    # 将几何增强后的PIL图像转换为numpy数组并保存为临时文件
    geometry_array = np.array(geometry_enhanced_image)
    temp_geometry_path = "temp_geometry_enhanced.png"
    cv2.imwrite(temp_geometry_path, cv2.cvtColor(geometry_array, cv2.COLOR_RGB2BGR))
    
    # 对几何增强后的图像进行深度估计
    enhanced_depth = depth_estimator.estimate_depth(temp_geometry_path, args.depth_dir)
    
    # 清理临时文件
    if os.path.exists(temp_geometry_path):
        os.remove(temp_geometry_path)
        
    # 初始化图像处理器
    image_processor = ImageProcessor()
    
    # 使用原始图像进行后续处理（不是增强后的图像）
    target_image = target_image_original  # 使用之前加载的原始图像
    ...
    # 加载材质图像
    texture_image = Image.open(texture_image_path).resize((1024,1024))
    ...
    ip_model = IPAdapterCustom(
        ...
        latent_adain_scale=config_dict.get('latent_adain_scale', 0.0)
    )
    ...
    # 2. 转为 NumPy 数组
    content_np = np.array(init_img)
    style_np = np.array(texture_image)
    # 3. 匹配颜色直方图（对每个通道分别匹配）
    matched_np = match_histograms(content_np, style_np, channel_axis=-1)

    # 4. 保存或返回结果
    init_img = Image.fromarray(np.uint8(matched_np))

    init_img = wct_color_transfer(init_img, texture_image)
    init_img.save('save.png')
