...
def wct_color_transfer(content_img, style_img, alpha=1.0):
    """简洁增强版WCT颜色迁移"""
    import cv2
    import numpy as np
    from PIL import Image
    ...
    # 计算协方差，增加正则化避免奇异
    c_cov = np.cov(c_centered) + np.eye(3) * 1e-3
    s_cov = np.cov(s_centered) + np.eye(3) * 1e-3

    # 奇异值分解（白化 + 着色）
    U_c, S_c, _ = np.linalg.svd(c_cov)
    U_s, S_s, _ = np.linalg.svd(s_cov)
    
    # 确保奇异值为正，增强数值稳定性
    S_c = np.maximum(S_c, 1e-6)
    ...
    # 应用强度控制
    if alpha < 1.0:
        colored = alpha * colored + (1 - alpha) * c_flat
