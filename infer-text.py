...
dsm = DirectionalShadingModule(
    ambient_strength=0.7,  # 环境光强度
    diffuse_strength=0.9  # 漫反射强度
)
...
# 选择光照方向（这里可以根据需要更换标签）
light_direction = 'dramatic'  # 可选: 'top', 'top_left', 'top_right', 'left', 'right', 'front', 'front_top', 'dramatic'
...
# pipe.load_textual_inversion("sd-concepts-library/doose-s-realistic-art-style")
# pipe.load_textual_inversion("sd-concepts-library/iridescent-photo-style")
...
num_samples = 1
prompt = "A highly realistic close-up of a traditional Chinese blue and white porcelain cup, featuring hand-painted cobalt blue patterns such as lotus flowers and swirling clouds on a smooth white ceramic body. The porcelain surface is glossy and slightly translucent, with subtle reflections and natural imperfections from handcrafted glazing. Fine craquelure and light refraction add to the realism. The blue pigment is absorbed slightly into the glaze, typical of authentic Qinghua ceramics. Studio lighting softly highlights the curvature and glaze depth. The focus is on the authentic texture, delicate linework, and ceramic luster."
# prompt = "glass,realistic,photograph,wall with text 'hello'"
images = ip_model.generate(prompt=prompt,scale=0.001, pil_image=ip_image, image=init_img, control_image=depth_map, mask_image=mask, controlnet_conditioning_scale=0.9, num_samples=num_samples, num_inference_steps=30, seed=42)
