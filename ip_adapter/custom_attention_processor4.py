...
class SemanticClipAttnProcessor(SemanticMaskedStyleAttnProcessor):
    def __init__(self, hidden_size, cross_attention_dim=None, scale=1.0, num_tokens=4, semantic_scale=0.5, latent_adain_scale=0.5):
        super().__init__(hidden_size, cross_attention_dim, scale, num_tokens)
        self.semantic_scale = semantic_scale
        self.latent_adain_scale = latent_adain_scale
    ...
    def __call__(self, attn, hidden_states, encoder_hidden_states=None, attention_mask=None, temb=None):
        ...
        if input_ndim == 4:
            batch_size, channel, height, width = hidden_states.shape
            hidden_states_reshaped = hidden_states.view(batch_size, channel, height * width).transpose(1, 2)
        else:
        ...
        if encoder_hidden_states is None:
            text_encoder_hidden_states = hidden_states_reshaped
        ...
        # --- Main Query ---
        query = attn.to_q(hidden_states_reshaped) # Shape: [B, L_q, D_inner]
        ...
        if ip_hidden_states is not None:
            ...
            # --- Latent AdaIN ---
            if self.latent_adain_scale > 0:
                ip_hidden_states_adain = ip_hidden_states_out.transpose(1, 2).reshape(batch_size, -1, height, width)
                text_hidden_states_adain = text_hidden_states.transpose(1, 2).reshape(batch_size, -1, height, width)
                adain_out = adain(text_hidden_states_adain, ip_hidden_states_adain)
                adain_out = adain_out.reshape(batch_size, inner_dim, height * width).transpose(1, 2)
                hidden_states = text_hidden_states + self.scale * ip_hidden_states_out + self.latent_adain_scale * (adain_out - text_hidden_states)
            else:
