...
from ip_adapter.custom_attention_processor4 import (
...
class IPAdapterCustom(IPAdapterXL):
    ...
    # ----------------- MODIFICATION START -----------------
    def __init__(self, sd_pipe, image_encoder_path, ip_ckpt, device, num_tokens=4, target_blocks=["block"], semantic_scale=0.5, latent_adain_scale=0.0):
            ...
            if cross_attention_dim is None:
                ...
                if selected:
                    ...
                    proc = SemanticClipAttnProcessor(
                        ...
                        latent_adain_scale=self.latent_adain_scale
                    ).to(self.device, dtype=torch.float16)
