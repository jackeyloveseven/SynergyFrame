...
class MultiScaleDepthEnhancement:
    ...
    def enhance(self, depth_map, reference_image):
        ...
        # AGGRESSIVE FIX: Apply Median Blur to the source depth map to remove initial banding.
        # This is the most critical step to eliminate the "contour line" artifacts.
        # We must convert to uint8 for the median blur function and then back to float32.
        depth_map_uint8 = np.clip(depth_map, 0, 255).astype(np.uint8)
        smoothed_depth = cv2.medianBlur(depth_map_uint8, 5)
        depth_map = smoothed_depth.astype(np.float32)
